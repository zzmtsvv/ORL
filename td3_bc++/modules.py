...
if __name__ == "__main__":
    ...
    print(repeated_states.shape)
    print(actor(repeated_states).size())

    random_actions = torch.rand(
        size=actor(repeated_states).size(),
        requires_grad=True,
        ) * 2.0 - actor.max_action

    current_q = critic(repeated_states, random_actions)
    print(current_q.shape)
    
    grad_q_wrt_random_action = torch.autograd.grad(
        outputs=current_q.sum(),
        inputs=random_actions,
        create_graph=True
    )[0].norm(p=2, dim=-1)
            
    gradient_penalty = F.relu(grad_q_wrt_random_action - 1).pow(2).mean()
