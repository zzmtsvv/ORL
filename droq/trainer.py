...
from config import droq_config
...
class DroQTrainer:
    ...
    def fit(self):
        ...
        with wandb.init(project=self.cfg.project, entity="zzmtsvv", group=self.cfg.group, name=self.cfg.name):
            ...
            for t in tqdm(range(self.cfg.max_timesteps), desc="DroQ steps"):
                
                batch = self.buffer.sample(self.batch_size)

                states, actions, rewards, next_states, dones = [x.to(self.device) for x in batch]

                logging_dict = self.droq.train(states,
                                                 actions,
                                                 rewards,
                                                 next_states,
                                                 dones)
                
                wandb.log(logging_dict, step=self.droq.total_iterations)