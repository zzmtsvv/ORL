...
class MSGTrainer:
    def __init__(self,
        ...
        critic = EnsembledCritic(self.state_dim, self.action_dim, cfg.hidden_dim, cfg.num_critics).to(self.device)
    ...
    def fit(self):
        ...
        with wandb.init(project=self.cfg.project, entity="zzmtsvv", group=self.cfg.group, name=self.cfg.name):
            ...
            for t in tqdm(range(self.cfg.max_timesteps), desc="MSG steps"):
                batch = self.buffer.sample(self.batch_size)
                
                states, actions, rewards, next_states, next_actions = [x.to(self.device) for x in batch]
                
                logging_dict = self.msg.train(states,
                                                    actions,
                                                    rewards,
                                                    next_states,
                                                    next_actions)
            
                wandb.log(logging_dict, step=self.msg.total_iterations)