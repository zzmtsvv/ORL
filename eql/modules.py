...
class Actor(nn.Module):
    ...
    def get_policy(self, state: torch.Tensor) -> Distribution:
        ...
        # log_std = self.min_log_std + (torch.tanh(self.log_std) + 1) * (self.max_log_std - self.min_log_std) / 2
...
if __name__ == "__main__":
    action = torch.rand(12, 6)
    state = torch.rand(12, 17)
    
    actor = Actor(17, 6)

    print(actor.log_prob(state, action).shape)
