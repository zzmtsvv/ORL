...
class RIQL:
    def __init__(self,
        ...
        self.critic = EnsembledCritic(cfg.state_dim,
                                      cfg.action_dim,
                                      cfg.hidden_dim,
                                      num_critics=cfg.num_critics).to(self.device)
        ...
        self.beta_huber = 1.0 / (self.sigma * self.sigma)
    ...
    def huber_loss(self, diff: torch.Tensor) -> torch.Tensor:
        diff = diff.abs()
        condition = diff < self.beta_huber
        loss = torch.where(condition, 0.5 * diff.pow(2) / self.beta_huber, diff - 0.5 * self.beta_huber)
        return loss.mean()
    ...
    def train(self,
        ...
        with torch.no_grad():
            ...
            v_target = torch.quantile(self.critic_target(states, actions), self.quantile, dim=0)
        ...
        td_error = current_q - q_target.squeeze(-1)
        # critic_loss = F.mse_loss(current_q, q_target.squeeze(-1))
        critic_loss = self.huber_loss(td_error)
